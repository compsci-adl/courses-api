...
def current_year() -> str:
    """Gets the current year, needed explicitly for the COURSE_DTL CoursePlanner API endpoint"""
...
def meeting_date_convert(raw_date: str) -> dict[str]:
    ...
    Returns:
        ...
        in a dict containing the "start" and "end" keys and their corresponding dates
    """
...
def parse_requisites(raw_requisites: str) -> Union[dict[str], None]:
    """Takes in a string of -requisites and returns a dict containing the original string,
    and a list of the parsed-out subjects

    Args:
        raw_requisites (str): The raw string containing a list of -requisites, usually in the
        format of "COMP SCI 1103, COMP SCI 2202, COMP SCI 2202B" as an example

    Returns:
        parsed_requisites (Union[dict[str], None]): Either a dict with "desc"s value being the
        original -requisites string and "subjects" being a list of the parsed -requisites,
    ...
    if matched_subjects:
        return {"desc": raw_requisites, "subjects": matched_subjects}
...
def convert_term_alias(term_alias: str) -> str:
    ...
    if term_alias not in terms_without_digits and term_alias[-1].isdigit() and \
            converted_alias != term_alias:
...
def get_term_number(year: int, term: str) -> int:
    ...
    Args:
        ...
        term (str): The term as a full string, for example "Semester 1" or
        "Semester 2", or as one of "sem1" or "sem2"
    ...
    terms_raw = DataFetcher(
        "TERMS/queryx&virtual=Y&year_from=0&year_to=9999").get()["data"]

    # Find and return the chosen term number
    for cur_term in terms_raw:
        if f"{year} {term}" == cur_term["DESCR"]:
...
def get_course_info(course_id: int, year: int, term: str) -> dict:
    ...
    Args:
        ...
        term (str): The term the course takes place in, either as "sem1", "sem2",
        "Semester 1", "Semester 2", or the ID of that term

    Returns:
        dict: A dict containing the course info per the spec, see issue #3 for more info
    """
    term_number = get_term_number(year, term)
    details = DataFetcher(f"COURSE_DTL/queryx&virtual=Y&year={
                          year}&courseid={course_id}&term={term_number}").get()["data"][0]
    classes = DataFetcher(f"COURSE_CLASS_LIST/queryx&virtual=Y&crseid={
                          course_id}&term={term_number}&offer=1&session=1").get()["data"][0]
    ...
    for class_detail in classes["groups"]:
        ...
        for _class in class_detail["classes"]:
            ...
            cur_class = {
                ...
                # Leaving this as it is in the API
    ...
    course_info = {
        ...
        "year": details["YEAR"],
        # Could use the "TERM" field but would then need to match the given term code
        "term": details["TERM_DESCR"],
        "campus": details["CAMPUS"],
        # Using a full text version instead of a code (eg UGRD)
        ...
        # Could parse to a float of hours, takes the format of "Up to [hours] hours per week"
...
def get_courses(year: int, term: str) -> list[dict]:
    """Takes in a year and term, and returns a list of all courses in that year
    and term as dicts described in the spec
    ...
    MAX_RESULTS = 5000
    term_url = "&term=" + get_term_number(year, term) if term else ""
    courses_raw = DataFetcher(f"COURSE_SEARCH/queryx&virtual=Y&year={
                              year}&pagenbr=1&pagesize={MAX_RESULTS}" + term_url).get()["data"]

    courses = []
    for course in courses_raw:
        courses.append({
            "course_id": course["COURSE_ID"],
            "name": f"{course['SUBJECT']} {course['CATALOG_NBR']}",
            # Below info was not specified, but I feel it could be useful
            "title": course["COURSE_TITLE"],
            "subject": course["SUBJECT"],
            "number": course["CATALOG_NBR"],
            # Could use the code instead (ex: UGRD instead of Undergraduate)
...
async def course_info_route(course_id, year: int = None, term: str = None):
    """Course details route, takes in a course ID (and optionally a year and term)
    and returns the courses' info and classes
...
async def courses_route(year: int = None, term: str = None):
    ...
    Args:
        year (int, optional): The year of the courses from 2006 to the
        current year. Defaults to None.
